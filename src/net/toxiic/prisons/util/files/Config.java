...
public class Config {
	...
	public static void setup(Plugin p) {
		...
		cfile = new File(p.getDataFolder(), "ranks.yml");
		if (!cfile.exists()) {
			try {
				cfile.createNewFile();
			} catch(IOException e) {
				Bukkit.getServer().getLogger().severe(Util.color("&cCould not create ranks.yml!"));
			}
		}
		ranks = YamlConfiguration.loadConfiguration(cfile);
		newDefault();
	}
	...
	public static void saveData() {
		try {
			...
			Bukkit.getServer().getLogger().severe(Util.color("&cCould not save ranks.yml!"));
		}
	}
	...
	public static void newDefault() {
		if (getData().getString("Ranks.Build") == null || !getData().getString("Ranks.Build").equals("1.0.1")) {
		      ...
		      setData("Config.Holograms", true);
		      setData("Config.Chat", true);
		      
		      setData("Config.AutoPickup.AllowedWorlds", worlds);

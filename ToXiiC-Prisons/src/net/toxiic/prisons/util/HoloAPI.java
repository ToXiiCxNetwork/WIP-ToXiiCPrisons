...
public class HoloAPI
{
    ...
    if (hasHolographicDisplays()) {
      try
      {
        if (hologramLocation != null)
        {
          ...
          return HolographicDisplaysAPI.createHologram(Main.getInstance(), hologramLocation, (String[])lines.toArray(new String[lines.size()]));
        }
      }
      catch (Exception ex) {}
    }
    ...
    if (hasHolographicDisplays()) {
      try
      {
        if (hologramLocation != null)
        {
          ...
          lines = MessageUtil.replace(lines);
          return HolographicDisplaysAPI.createIndividualHologram(Main.getInstance(), hologramLocation, Arrays.asList(new Player[] { player }), lines.isEmpty() ? new String[] { "" } : (String[])lines.toArray(new String[lines.size()]));
        }
      }
      catch (Exception ex) {}
    }
  ...
  public static Hologram[] getHolograms()
  {
    if (hasHolographicDisplays()) {
      try
      {
        return HolographicDisplaysAPI.getHolograms(Main.getInstance());
      }
      catch (Exception ex) {}
    }
    return null;
  ...
  {
    return (Bukkit.getServer().getPluginManager().isPluginEnabled("HolographicDisplays")) && (Main.getInstance().getConfig().getBoolean("Config.Holograms", true));
  }
}
